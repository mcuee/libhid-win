    ...
    unsigned int const depth, char* const buffer, unsigned int const size)
{
  ...
  if (!hid_is_opened(hidif)) {
    WARNING_PRINT("the device has not been opened.");
    return HID_RET_DEVICE_NOT_OPENED;
  }

  TRACE_PRINT("looking up report ID...");
  hidif->hid_data->Type = ITEM_INPUT;
  hidif->hid_data->ReportID = 0;

  hid_find_object(hidif, path, depth, 0);

  TRACE_PRINT("retrieving report ID 0x%02x (length: %d) from USB device %s...", 
  ...
  if (len < 0) {
    WARNING_PRINT("failed to retrieve report from USB device %s:%s.", hidif->id, usb_strerror());
    return HID_RET_FAIL_GET_REPORT;
  }

  if (len != (signed)size) {
    WARNING_PRINT("failed to retrieve complete report from USB device %s; "
        "requested: %d bytes, got: %d bytes.", hidif->id, size, len);
    return HID_RET_FAIL_GET_REPORT;
  }

  NOTICE_PRINT("successfully retrieved report from USB device %s.", hidif->id);
    ...
    unsigned int const depth, char const* const buffer, unsigned int const size)
{
  ...
  hid_find_object(hidif, path, depth, size);

  TRACE_PRINT("sending report ID 0x%02x (length: %d) to USB device %s...", 
        hidif->hid_data->ReportID, size, hidif->id);
  ...
  if (len < 0) {
    WARNING_PRINT("failed to send report to USB device %s:%s.", hidif->id,usb_strerror());
    return HID_RET_FAIL_SET_REPORT;
  }

  if (len != (signed)size) {
    WARNING_PRINT("failed to send complete report to USB device %s; "
        "requested: %d bytes, sent: %d bytes.", hidif->id, 
        size, len);
    return HID_RET_FAIL_SET_REPORT;
  }

  NOTICE_PRINT("successfully sent report to USB device %s.", hidif->id);
  return HID_RET_SUCCESS;
}
    ...
    unsigned int const depth, double *const value)
{
  ...
  TRACE_PRINT("retrieving report from USB device %s...", hidif->id);
  hidif->hid_data->Type = ITEM_FEATURE;
  hidif->hid_data->ReportID = 0;
 ...
 * @param[in/out] size   How many bytes to read/How many bytes were read
...
hid_return hid_interrupt_read(HIDInterface * const hidif,
     unsigned int const ep, char* const bytes, unsigned int *size, 
     unsigned int const timeout) 
{
  ...
  TRACE_PRINT("retrieving interrupt report from device %s ...", hidif->id);

  len = usb_interrupt_read(hidif->dev_handle,
                               ep,
                               bytes,
                               *size,
                               timeout);
  ...
  *size = 0;
  ...
  if (len == -ETIMEDOUT) {
    WARNING_PRINT("timeout on interrupt read from device %s", hidif->id);
    return HID_RET_TIMEOUT;
  }

  if (len <= 0) {
    WARNING_PRINT("failed to get interrupt read from device %s: %s", hidif->id, usb_strerror());
    return HID_RET_FAIL_INT_READ;
  }

  *size = (unsigned int)len;

  NOTICE_PRINT("successfully got interrupt report from device %s", hidif->id);
  return HID_RET_SUCCESS;
}
     ...
     unsigned int const timeout) 
{
  ...
  TRACE_PRINT("writing interrupt report to device %s ...", hidif->id);

  len = usb_interrupt_write(hidif->dev_handle,
                               ep,
                               (char *)bytes,
                               size,
                               timeout);
  ...
  if (len == -ETIMEDOUT) {
    WARNING_PRINT("timeout on interrupt write to device %s", hidif->id);
    return HID_RET_TIMEOUT;
  }

  if (len < 0) {
    WARNING_PRINT("failed to perform interrupt write to device %s: %s", hidif->id, usb_strerror());
    return HID_RET_FAIL_INT_READ;
  }

  if (len != (signed)size) {
    WARNING_PRINT("failed to write all of interrupt report to device %s; "
      "requested: %d bytes, sent: %d bytes.", hidif->id,
      size, len);
    return HID_RET_FAIL_INT_READ;
  }

  NOTICE_PRINT("successfully sent interrupt report to device %s", hidif->id);
  return HID_RET_SUCCESS;
}
		...
		unsigned duration, unsigned report_id) 
{
  ...
  if(duration > 255) {
    WARNING_PRINT("duration must be in the range [0,255]");
    return HID_RET_INVALID_PARAMETER;
  }

  if(report_id > 255) {
    WARNING_PRINT("Report ID must be in the range [0,255]");
    return HID_RET_INVALID_PARAMETER;
  }
  ...
  if (len != 0) {
    WARNING_PRINT("failed to Set_Idle for USB device %s:%s.", hidif->id, usb_strerror());
    return HID_RET_FAIL_GET_REPORT;
  }
