...
HIDInterface* hid_new_HIDInterface()
{
  ...
  TRACE_PRINT("creating a new HIDInterface instance...");

  ret = (HIDInterface*)malloc(sizeof(HIDInterface));
  if (!ret) {
    ERROR_PRINT("could not allocate memory for HIDInterface instance.");
    return 0;
  }
...
{
  if (!ixs || !*ixs) {
    ERROR_PRINT("cannot delete NULL HIDInterface.");
    return;
  }
...
{
  if (!hidif) {
    ERROR_PRINT("cannot reset NULL HIDInterface.");
    return;
  }
...
hid_return hid_init()
{
  if (hid_is_initialised()) {
    ERROR_PRINT("cannot initialised already initialised HID library");
    return HID_RET_ALREADY_INITIALISED;
  }

  /* Include version to make deciphering logfiles easier: */
  NOTICE_PRINT(PACKAGE_STRING " is being initialized.");
  
  TRACE_PRINT("initialising USB subsystem...");
  usb_init();

  TRACE_PRINT("scanning for USB busses...");
  if (usb_find_busses() < 0) {
    ERROR_PRINT("failed to scan for USB busses");
    return HID_RET_FAIL_FIND_BUSSES;
  }

  TRACE_PRINT("scanning for USB devices...");
  if (usb_find_devices() < 0) {
    ERROR_PRINT("failed to scan for USB devices");
    return HID_RET_FAIL_FIND_DEVICES;
  }

  initialised = true;

  NOTICE_PRINT("successfully initialised HID library.");
...
{
  if (!hid_is_initialised()) {
    ERROR_PRINT("cannot cleanup uninitialised HID library.");
    return HID_RET_NOT_INITIALISED;
  }

  initialised = false;
  NOTICE_PRINT("successfully deinitialised HID library.");
  return HID_RET_SUCCESS;
}
