    ...
    HIDInterface const* const hidif)
{
  ...
  if (!hid_is_opened(hidif)) {
    ERROR_PRINT("cannot write identification of unopened HIDinterface.");
    return HID_RET_DEVICE_NOT_OPENED;
  }

  if (!out) {
    ERROR_PRINT("cannot write HIDinterface identification of USB device %s "
  ...
  if (hidif->device->descriptor.iManufacturer) {
    len = usb_get_string_simple(hidif->dev_handle,
        hidif->device->descriptor.iManufacturer, buffer, sizeof(buffer));
  ...
  if (hidif->device->descriptor.iProduct) {
    len = usb_get_string_simple(hidif->dev_handle,
        hidif->device->descriptor.iProduct, buffer, sizeof(buffer));
  ...
  if (hidif->device->descriptor.iSerialNumber) {
    len = usb_get_string_simple(hidif->dev_handle,
        hidif->device->descriptor.iSerialNumber, buffer, sizeof(buffer));
...
hid_return hid_dump_tree(FILE* const out, HIDInterface* const hidif)
{
  ...
  if (!hid_is_opened(hidif)) {
    ERROR_PRINT("cannot dump tree of unopened HIDinterface.");
    return HID_RET_DEVICE_NOT_OPENED;
  }

  if (!out) {
    ERROR_PRINT("cannot dump HIDinterface tree of USB device %s to NULL output stream.",
        hidif->id);
    return HID_RET_INVALID_PARAMETER;
  }
  ...
  TRACE_PRINT("iterating the parse tree for USB device %s...", hidif->id);
...
const char *hid_strERROR_PRINT(hid_return ret)
{
  switch(ret) {
  ...
  return "libhid: unknown ERROR_PRINT";
}
