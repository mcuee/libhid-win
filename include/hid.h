 ...
 * here are most likely for internal use only, and may change without NOTICE_PRINT.
 */
...
typedef enum hid_return_t {
  ...
  HID_RET_NOT_FOUND,
  HID_RET_TIMEOUT,
  HID_RET_FAIL_SET_CONFIG,
  HID_RET_FAIL_ALT_IFACE
} hid_return;
...
hid_return hid_interrupt_read(HIDInterface* const hidif, unsigned int const ep,
    char* const bytes, unsigned int *size, unsigned int const timeout);
