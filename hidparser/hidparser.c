...
int HIDParse(HIDParser* pParser, HIDData* pData)
{
  ...
  while(!Found && pParser->Pos<pParser->ReportDescSize)
  {
    ...
    switch(pParser->Item & ITEM_MASK)
    {
      ...
      case ITEM_COLLECTION :
      {
        ...
        if(pParser->Value>=0x80)
        {
          ...
          pParser->Data.Path.Node[pParser->Data.Path.Size].Usage=(ushort)(pParser->Value & 0x7F);
          pParser->Data.Path.Size++;
        }
	...
	pParser->Pos+=(uchar)(pParser->Value & 0xff);
      }
    }
  } /* while(!Found && pParser->Pos<pParser->ReportDescSize) */

  ERROR_PRINT(pParser->Data.Path.Size>=PATH_SIZE);
  ERROR_PRINT(pParser->ReportDescSize>=REPORT_DSC_SIZE);
  ERROR_PRINT(pParser->UsageSize>=USAGE_TAB_SIZE);
  ERROR_PRINT(pParser->Data.ReportID>=MAX_REPORT);
 ...
 */
int FindObject(HIDParser* pParser, HIDData* pData, unsigned int size)
{
  HIDData FoundData;
  ResetParser(pParser);
  while(HIDParse(pParser, &FoundData))
  {
    if(pData->Path.Size>0 && 
      ...
      (((unsigned int)pParser->ReportCount) >= size || 0 == size) &&
